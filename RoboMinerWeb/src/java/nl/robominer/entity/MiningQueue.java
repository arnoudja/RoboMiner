...
@NamedQueries(
{
    ...
    @NamedQuery(name = "MiningQueue.findWaitingByUsersId",
                query = "SELECT m FROM MiningQueue m WHERE (m.miningEndTime IS NULL OR m.miningEndTime > CURRENT_TIMESTAMP) AND m.robot.user.id = :usersId ORDER BY m.id"),
    @NamedQuery(name = "MiningQueue.findWaitingByRobotId",
                query = "SELECT m FROM MiningQueue m WHERE (m.miningEndTime IS NULL OR m.miningEndTime > CURRENT_TIMESTAMP) AND m.robot.id = :robotId ORDER BY m.id"),
    @NamedQuery(name = "MiningQueue.findResultsByRobotId",
                query = "SELECT m FROM MiningQueue m WHERE m.claimed = true AND m.robot.id = :robotId ORDER BY m.miningEndTime DESC"),
