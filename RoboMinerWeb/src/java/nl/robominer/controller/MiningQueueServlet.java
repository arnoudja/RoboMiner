...
public class MiningQueueServlet extends RoboMinerServletBase
{
            ...
            throws ServletException, IOException
    {
        ...
        String[] selectedQueueItems = request.getParameterValues(
                "selectedQueueItemId");
        ...
        if (submitType != null)
        {
            ...
            {
            case "add":
                errorMessage = addMiningQueueItem(request,
                                                  user.getRobot(robotId),
                                                  user.getMiningArea(miningAreaAddId),
                                                  false);
                break;
            ...
            case "remove":
                ...
                {
                    removeMiningQueueItems(user.getId(), robotId,
                                           selectedQueueItems);
                }
                break;
            }
        ...
        {
            List<MiningQueueItem> robotQueueList = getQueueInfo(
                    miningQueueFacade.findWaitingByRobotId(robot.getId()),
                    selectedQueueItems);
        ...
        if (infoMiningAreaId <= 0)
        {
            ...
            {
                infoMiningAreaId = (int)request.getSession().getAttribute(
                        SESSION_INFO_MINING_AREA_ID);
            }
        ...
        Map<Integer, Integer> sessionRobotMiningAreaIdMap = (Map<Integer, Integer>)request
                .getSession().getAttribute(SESSION_ROBOT_MINING_AREA_ID_MAP);
        Map<Integer, Integer> robotMiningAreaIdMap = new HashMap<>();

        for (Robot robot : user.getRobotList())
        {
            int selectedMiningAreaId = getItemId(request, "miningArea" +
                                                 robot.getId());

            if (selectedMiningAreaId <= 0)
            {
                if (sessionRobotMiningAreaIdMap != null &&
                    sessionRobotMiningAreaIdMap.get(robot.getId()) != null)
                {
                    selectedMiningAreaId = sessionRobotMiningAreaIdMap.get(
                                                                robot.getId());
                }
                else if (!user.getMiningAreaList().isEmpty())
                {
                    selectedMiningAreaId = user.getMiningAreaList().get(0)
                            .getId();
                }
            }
        ...
        request.getSession().setAttribute(SESSION_ROBOT_MINING_AREA_ID_MAP,
                                          robotMiningAreaIdMap);
        request.getSession().setAttribute(SESSION_INFO_MINING_AREA_ID,
                                          infoMiningAreaId);
     ...
     */
    private String addMiningQueueItem(HttpServletRequest request, Robot robot,
                                      MiningArea miningArea, boolean fill) throws
            ServletException
    {
        String errorMessage = null;
        ...
        {
            List<MiningQueue> robotMiningQueueList = miningQueueFacade
    ...
    {
        List<MiningQueueItem> miningQueueList = getQueueInfo(miningQueueFacade
                .findWaitingByUsersId(userId), selectedQueueItems);

        for (MiningQueueItem item : miningQueueList)
        {
            if (item.isSelected() &&
                item.getMiningQueue().getRobot().getId() == robotId &&
                item.getItemStatus() == EMiningQueueItemStatus.QUEUED)
            {
                                               ...
                                               String[] selectedItems)
    {
        ...
        for (MiningQueue miningQueue : miningQueueList)
        {
            ...
            if (!robotTimeLeft.containsKey(robot.getId()))
            {
                ...
                {
                    long timeLeft = (robot.getRechargeEndTime().getTime() -
                                     now.getTime()) / 1000;
                    if (timeLeft <= 0)
                    {
                        timeLeft = 1;
                    ...
                    result.add(new MiningQueueItem(miningQueue,
                                                   EMiningQueueItemStatus.RECHARGING,
                                                   timeLeft, selected));
                ...
                else
                {
                    ...
                    if (robot.getMiningEndTime() != null &&
                        robot.getMiningEndTime().after(now))
                    {
                        timeLeft = (robot.getMiningEndTime().getTime() - 
                                    now.getTime()) / 1000;
                    }
                    else
                    {
                        Date miningStart = robot.getRechargeEndTime().after(
                                miningQueue.getCreationTime()) ?
                                           robot.getRechargeEndTime() :
                                           miningQueue.getCreationTime();

                        timeLeft = (miningStart.getTime() - now.getTime()) /
                                   1000 +
                                   miningQueue.getMiningArea().getMiningTime();
                    }

                    if (timeLeft > 0)
                    {
                        result.add(new MiningQueueItem(miningQueue,
                                                       EMiningQueueItemStatus.MINING,
                                                       timeLeft, selected));
                    ...
                    {
                        result.add(new MiningQueueItem(miningQueue,
                                                       EMiningQueueItemStatus.UPDATING,
                                                       1, selected));
            ...
            else
            {
                ...
                timeLeft += miningQueue.getRobot().getRechargeTime() +
                        miningQueue.getMiningArea().getMiningTime();

                result.add(new MiningQueueItem(miningQueue,
                                               EMiningQueueItemStatus.QUEUED,
                                               timeLeft, selected));
